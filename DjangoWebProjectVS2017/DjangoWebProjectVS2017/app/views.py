...
def choice_add(request, question_id):
        ...
        if request.method =='POST':
            ...
            if(AuxCorrect):
                if(choice.correct):
                    return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form,'enunciado': 'Numero de opciones de la pregunta: ','opciones': '57','lleno': 'La pregunta ya contiene una opción correcta'})
                else:
                    if form.is_valid():
                        choice = form.save(commit = False)
